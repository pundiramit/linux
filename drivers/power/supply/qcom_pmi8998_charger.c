...
#define PRE_CHARGE_CURRENT_CFG_REG			0x60
...
static int smb2_set_current_limit(struct smb2_chip *chip, unsigned int val)
{
	...
	dev_info(chip->dev, "CA:: %s() icl_option = 0x%x\n", __func__,
		 icl_option);
	rc = regmap_update_bits(
		chip->regmap, chip->base + USBIN_ICL_OPTIONS_REG,
		CFG_USB3P0_SEL_BIT | USB51_MODE_BIT | USBIN_MODE_CHG_BIT,
		icl_option);

	if (icl_option && USBIN_MODE_CHG_BIT)
		rc = rc	   ?:
				   regmap_update_bits(chip->regmap,
						chip->base + USBIN_LOAD_CFG_REG,
	...
	regmap_read(chip->regmap, chip->base + USBIN_LOAD_CFG_REG, &load_cfg);
	dev_info(chip->dev, "USBIN_LOAD_CFG_REG = 0x%x\n", load_cfg);
...
static ssize_t smb2_debug_write(struct file *file, const char __user *buf,
				size_t count, loff_t *ppos)
{
	struct smb2_chip *chip = file->private_data;
	int rc;
	unsigned long long addr;
	unsigned int reg;

	rc = kstrtoull_from_user(buf, count, 0, &addr);
	if (rc < 0)
		return rc;

	rc = regmap_read(chip->regmap, chip->base + (u16)addr, &reg);
	if (rc < 0)
		return rc;

	dev_info(chip->dev, "SMB2 DEBUG: (addr = 0x%04x, val = 0x%02x)\n", addr + 0x1000,
		 reg);

	return count;
}

struct file_operations smb2_addr_debug_ops = {
	.open = simple_open,
	.write = smb2_debug_write,
};

static struct dentry *smb2_create_debugfs(struct smb2_chip *chip)
{
	chip->debug_file = debugfs_create_file("addr", 0666, NULL, chip,
					       &smb2_addr_debug_ops);
	return chip->debug_file;
}

static void smb2_remove_debugfs(struct smb2_chip *chip)
{
	debugfs_remove(chip->debug_file);
}
#else
static inline struct dentry *smb2_create_debugfs(struct smb2_chip *chip)
{
	return NULL;
}
static inline void smb2_remove_debugfs(struct smb2_chip *chip)
{
}
#endif
